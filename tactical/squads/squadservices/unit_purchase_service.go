...
func (ups *UnitPurchaseService) CanPurchaseUnit(playerID ecs.EntityID, template unitdefs.UnitTemplate) *PurchaseValidationResult {
	...
	// Get player roster
	roster := rstr.GetPlayerRoster(playerID, ups.entityManager)
	if roster == nil {
...
func (ups *UnitPurchaseService) PurchaseUnit(playerID ecs.EntityID, template unitdefs.UnitTemplate) *PurchaseResult {
	...
	unitEntity, err := squadcore.CreateUnitEntity(ups.entityManager, template)
	if err != nil {
