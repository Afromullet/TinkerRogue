...
func (c *ChangeLeaderCommand) Validate() error {
	...
	isLeader := c.manager.HasComponent(c.newLeaderID, squadcore.LeaderComponent)
	if isLeader {
...
func (c *ChangeLeaderCommand) Execute() error {
	// Find current leader
	c.oldLeaderID = squadcore.GetLeaderID(c.squadID, c.manager)
	...
	if c.oldLeaderID != 0 {
		...
		if oldLeaderEntity != nil {
			squadcore.RemoveLeaderComponents(oldLeaderEntity)
		}
	}
	...
	squadcore.AddLeaderComponents(newLeaderEntity)

	// Update squad renderable to show the new leader's sprite
	squadEntity := squadcore.GetSquadEntity(c.squadID, c.manager)
	if squadEntity != nil {
		squadcore.SetSquadRenderableFromLeader(c.squadID, squadEntity, c.manager)
	}
...
func (c *ChangeLeaderCommand) Undo() error {
	...
	newLeaderEntity := c.manager.FindEntityByID(c.newLeaderID)
	if newLeaderEntity != nil {
		squadcore.RemoveLeaderComponents(newLeaderEntity)
	}
	...
	if c.oldLeaderID != 0 {
		oldLeaderEntity := c.manager.FindEntityByID(c.oldLeaderID)
		if oldLeaderEntity != nil {
			squadcore.AddLeaderComponents(oldLeaderEntity)
		}
	}
