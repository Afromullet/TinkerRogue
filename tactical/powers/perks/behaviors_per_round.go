// behaviors_per_round.go — per-round stateful perk behaviors.
//
// These behaviors read/write PerkRoundState — either the shared tracking
// fields (MovedThisTurn, AttackedThisTurn, etc.) or the per-round PerkState
// map. State clears at round boundaries via ResetPerkRoundStateRound. See
// the package comment in behaviors_stateless.go for the taxonomy split.
package perks
