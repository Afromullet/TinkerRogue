...
func GetUnitPurchaseResourceLabels(panels *framework.PanelRegistry) (gold, roster *widget.Text) {
	if result := panels.Get(UnitPurchasePanelResourceDisplay); result != nil {
		if g, ok := result.Custom["goldLabel"].(*widget.Text); ok {
			gold = g
		}
		if r, ok := result.Custom["rosterLabel"].(*widget.Text); ok {
			roster = r
		}
	}
	return
...
func GetUnitPurchaseUnitList(panels *framework.PanelRegistry) *widgets.CachedListWrapper {
	if result := panels.Get(UnitPurchasePanelUnitList); result != nil {
		if list, ok := result.Custom["unitList"].(*widgets.CachedListWrapper); ok {
			return list
		}
	}
	return nil
...
func GetUnitPurchaseDetailWidgets(panels *framework.PanelRegistry) (detailArea, statsArea *widgets.CachedTextAreaWrapper, viewStatsBtn *widget.Button) {
	if result := panels.Get(UnitPurchasePanelDetailPanel); result != nil {
		if d, ok := result.Custom["detailTextArea"].(*widgets.CachedTextAreaWrapper); ok {
			detailArea = d
		}
		if s, ok := result.Custom["statsTextArea"].(*widgets.CachedTextAreaWrapper); ok {
			statsArea = s
		}
		if b, ok := result.Custom["viewStatsButton"].(*widget.Button); ok {
			viewStatsBtn = b
		}
	}
	return
...
func GetUnitPurchaseBuyButton(panels *framework.PanelRegistry) *widget.Button {
	if result := panels.Get(UnitPurchasePanelActionButtons); result != nil {
		if btn, ok := result.Custom["buyButton"].(*widget.Button); ok {
			return btn
		}
	}
	return nil
