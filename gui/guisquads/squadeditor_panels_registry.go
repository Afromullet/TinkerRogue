...
// GetSquadEditorNavButtons returns the navigation buttons from the panel registry
func GetSquadEditorNavButtons(panels *framework.PanelRegistry) (prev, next *widget.Button, counter *widget.Text) {
	if result := panels.Get(SquadEditorPanelNavigation); result != nil {
		if p, ok := result.Custom["prevButton"].(*widget.Button); ok {
			prev = p
		}
		if n, ok := result.Custom["nextButton"].(*widget.Button); ok {
			next = n
		}
		if c, ok := result.Custom["counterLabel"].(*widget.Text); ok {
			counter = c
		}
	}
	return
...
// GetSquadEditorSquadList returns the squad selector list from the panel registry
func GetSquadEditorSquadList(panels *framework.PanelRegistry) *widget.List {
	if result := panels.Get(SquadEditorPanelSquadSelector); result != nil {
		if list, ok := result.Custom["squadList"].(*widget.List); ok {
			return list
		}
	}
	return nil
...
// GetSquadEditorGridCells returns the 3x3 grid cells from the panel registry
func GetSquadEditorGridCells(panels *framework.PanelRegistry) [3][3]*widget.Button {
	if result := panels.Get(SquadEditorPanelGridEditor); result != nil {
		if cells, ok := result.Custom["gridCells"].([3][3]*widget.Button); ok {
			return cells
		}
	}
...
// GetSquadEditorUnitList returns the unit list from the panel registry
func GetSquadEditorUnitList(panels *framework.PanelRegistry) *widget.List {
	if result := panels.Get(SquadEditorPanelUnitList); result != nil {
		if list, ok := result.Custom["unitList"].(*widget.List); ok {
			return list
		}
	}
	return nil
...
// GetSquadEditorRosterList returns the roster list from the panel registry
func GetSquadEditorRosterList(panels *framework.PanelRegistry) *widget.List {
	if result := panels.Get(SquadEditorPanelRosterList); result != nil {
		if list, ok := result.Custom["rosterList"].(*widget.List); ok {
			return list
		}
	}
	return nil
