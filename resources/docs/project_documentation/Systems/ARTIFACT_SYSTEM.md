...
`CombatService.TeardownCombat` removes all active effects from all units (including those applied by artifacts), strips combat-only state from player squads (faction membership, perk round state, positions, IsDeployed), and disposes enemy entities. The charge tracker is not disposed — it is reset at the start of the next battle. Artifact equipment assignments on squads persist across battles; the `EquipmentComponent` is not touched during cleanup.
