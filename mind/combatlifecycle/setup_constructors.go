...
// Type is fixed to CombatTypeOverworld; per-type-only fields (DefendedNodeID,
// PostCombatReturnMode) stay at zero values so they cannot be set for the wrong type.
...
// PostCombatReturnMode is wired to PostCombatReturnRaid so the post-combat flow
// returns to raid mode rather than the default exploration mode.
func NewRaidSetup(
	playerFactionID ecs.EntityID,
	enemyFactionID ecs.EntityID,
	enemySquadIDs []ecs.EntityID,
	...
	return &CombatSetup{
		...
		RosterOwnerID:        rosterOwnerID,
		Type:                 CombatTypeRaid,
		PostCombatReturnMode: PostCombatReturnRaid,
		Resolver:             resolver,
	}
}
