...
func TestGetThrowableItems(t *testing.T) {
	service, ecsManager, playerID := setupTestInventoryService()
	...
	// Add throwable and non-throwable items
	throwable1 := createTestThrowableItem(ecsManager.World, "Fire Bomb")
	throwable2 := createTestThrowableItem(ecsManager.World, "Ice Bomb")
	nonThrowable := createTestNonThrowableItem(ecsManager.World, "Potion")

	AddItem(ecsManager.World, inv, throwable1.GetID())
	AddItem(ecsManager.World, inv, nonThrowable.GetID())
	AddItem(ecsManager.World, inv, throwable2.GetID())

	// Get only throwable items
	throwables := service.GetThrowableItems(playerID)

	// Verify only throwables are returned
	if len(throwables) != 2 {
		t.Errorf("Expected 2 throwable items, got %d", len(throwables))
	}

	// Verify item names
	foundFire := false
	foundIce := false
	for _, item := range throwables {
		if item.Name == "Fire Bomb" {
			foundFire = true
		}
		if item.Name == "Ice Bomb" {
			foundIce = true
		}
	}

	if !foundFire {
		t.Error("Expected to find 'Fire Bomb' in throwables")
	}
	if !foundIce {
		t.Error("Expected to find 'Ice Bomb' in throwables")
	}
}

func TestHasThrowableItems(t *testing.T) {
	service, ecsManager, playerID := setupTestInventoryService()
	...
	// Initially no throwables
	if service.HasThrowableItems(playerID) {
		t.Error("Expected no throwable items initially")
	}
	...
	// Still no throwables
	if service.HasThrowableItems(playerID) {
		t.Error("Expected no throwable items after adding potion")
	}

	// Add throwable item
	throwable := createTestThrowableItem(ecsManager.World, "Fire Bomb")
	AddItem(ecsManager.World, inv, throwable.GetID())

	// Now should have throwables
	if !service.HasThrowableItems(playerID) {
		t.Error("Expected to have throwable items after adding Fire Bomb")
	}
}
