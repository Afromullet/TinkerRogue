...
### 1.1 Cache SquadInfo Queries During Render Frame (5-6 hours)

**Impact**: -5s (4.7% CPU time saved)
**Current**: `GetSquadInfo` called 200+ times per frame via SquadHighlightRenderer

**Problem Analysis**:
```
SquadHighlightRenderer.Render: 6,298ms (5.43%)
├── GetSquadInfo: 5,454ms (86.6%)  ⚠️ Called for EVERY squad, EVERY frame
│   └── GetSquadInfoCached: 5,468ms
│       ├── GetAttributesByIDWithTag: 4,009ms (73%)
│       │   └── GetComponentTypeByID: 4,746ms
│       │       └── GetEntityByID: 4,673ms (creates new objects!)
│       └── newobject: 686ms
└── DrawTileBorder: 593ms
```

**Solution**: Frame-level squad info cache

```go
// gui/guicomponents/framecache.go (NEW FILE)
...
// FrameCache caches data that doesn't change within a single frame
type FrameCache struct {
    squadInfoCache map[ecs.EntityID]*SquadInfo
    frameNumber    uint64
    queries        *GUIQueries
}

func NewFrameCache(queries *GUIQueries) *FrameCache {
    return &FrameCache{
        squadInfoCache: make(map[ecs.EntityID]*SquadInfo, 20),
        frameNumber:    0,
        queries:        queries,
    }
}

// BeginFrame clears the cache for a new frame
func (fc *FrameCache) BeginFrame() {
    fc.frameNumber++
    // Reuse map - just clear it
    for k := range fc.squadInfoCache {
        delete(fc.squadInfoCache, k)
    }
}

// GetSquadInfo returns cached squad info, or fetches and caches it
func (fc *FrameCache) GetSquadInfo(squadID ecs.EntityID) *SquadInfo {
    if info, ok := fc.squadInfoCache[squadID]; ok {
        return info  // Cache hit - FREE
    }

    // Cache miss - fetch once per frame
    info := fc.queries.GetSquadInfo(squadID)
    if info != nil {
        fc.squadInfoCache[squadID] = info
    }
    return info
}
```
...
**Integration**:
```go
// gui/guimodes/squadhighlightrenderer.go
type SquadHighlightRenderer struct {
    // ...existing fields...
    frameCache *guicomponents.FrameCache
}

func (shr *SquadHighlightRenderer) Render(screen *ebiten.Image) {
    // Use frame cache instead of direct queries
    for _, squadID := range squadIDs {
        squadInfo := shr.frameCache.GetSquadInfo(squadID)  // Cached!
        // ...
    }
}
```
...
**Expected Result**:
- **Before**: 5,454ms calling GetSquadInfo hundreds of times
- **After**: ~100ms calling GetSquadInfo once per squad
- **Savings**: 5.3s (5.3s / 120s = 4.4% CPU time)
