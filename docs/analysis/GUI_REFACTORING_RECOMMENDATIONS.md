...
### 1.1 Mode Initialization Template Pattern

**Problem:** Every mode has nearly identical initialization boilerplate (70-100 lines):
```go
// Repeated in ~10 mode files
func (m *Mode) Initialize(ctx *core.UIContext) error {
    m.InitializeBase(ctx)

    // Build panels using factory
    panel1 := m.uiFactory.CreatePanel1()
    m.RootContainer.AddChild(panel1)

    panel2 := m.uiFactory.CreatePanel2()
    m.RootContainer.AddChild(panel2)

    // Build action buttons
    buttonSpecs := []widgets.ButtonSpec{...}
    actionButtons := gui.CreateActionButtonGroup(...)
...
**Impact:**
- **Duplication:** 600+ lines of similar initialization code across modes
- **Maintenance:** Changing panel creation patterns requires updating 10+ files
- **Errors:** Easy to miss steps (e.g., forgetting AddChild, inconsistent ordering)

**Recommendation:** Create `ModeBuilder` pattern with declarative configuration
...
type ModeConfig struct {
    ...
    StatusLabel bool
    Commands    bool // Enable command history
}

type PanelSpec struct {
    SpecName    string           // From StandardPanels
    CustomBuild func() *widget.Container // Optional override
    OnCreate    func(*widget.Container) // Post-creation hook
}
...
// ModeBuilder constructs modes declaratively
type ModeBuilder struct {
    baseMode *BaseMode
    config   ModeConfig
}

func NewModeBuilder(base *BaseMode, config ModeConfig) *ModeBuilder {
    return &ModeBuilder{baseMode: base, config: config}
}
...
func (mb *ModeBuilder) Build(ctx *core.UIContext) error {
    ...
    // Build panels
    for _, panelSpec := range mb.config.Panels {
    ...
    // Build button groups
    for _, btnGroup := range mb.config.Buttons {
        container := gui.CreateActionButtonGroup(
    ...
    if mb.config.Commands {
        mb.baseMode.InitializeCommandHistory(func() {
            // Override in mode-specific code
        })
    }
...
func (em *ExplorationMode) Initialize(ctx *core.UIContext) error {
    return NewModeBuilder(&em.BaseMode, ModeConfig{
        ModeName:   "exploration",
        ReturnMode: "", // No return mode for main exploration
        Hotkeys: []HotkeySpec{
        ...
        Panels: []PanelSpec{
            ...
            {SpecName: "quick_inventory", CustomBuild: em.buildQuickInventory},
        },
        Buttons: []ButtonGroupSpec{
            {
                Position: widgets.BottomCenter(),
                Buttons: []widgets.ButtonSpec{
                    {Text: "Squads (E)", OnClick: em.handleSquadsClick},
                    {Text: "Inventory (I)", OnClick: em.handleInventoryClick},
                },
            },
        },
    }).Build(ctx)
...
**Benefits:**
- Reduces mode initialization from 70-100 lines to 20-40 lines
- Eliminates 600+ lines of duplicated code
- Makes mode structure immediately visible
- Easier to add new modes (copy config, modify)
- Centralized changes to initialization patterns

---

### 1.2 Consolidate Factory Patterns

**Problem:** Multiple factory approaches create confusion and duplication:

1. **CombatUIFactory** (guicombat/combat_ui_factory.go) - Instance-based factory
2. **SquadBuilderUIFactory** (guisquads/squad_builder_ui_factory.go) - Instance-based factory
3. **Panel builders** (widgets/panels.go, widgets/panel_factory.go) - Functional builders
4. **Widget creators** (widgets/createwidgets.go) - Global functions
5. **List helpers** (widgets/list_helpers.go) - Config-based creators

**Impact:**
- **Confusion:** Developers must remember which pattern to use when
- **Duplication:** Similar logic scattered across 5 different files
- **Inconsistency:** Some use PanelBuilders, some don't; some take Layout, some take Context

**Recommendation:** Consolidate to TWO patterns

**Pattern 1: UIComponentFactory (for complex, stateful UI)**
```go
// gui/core/componentfactory.go
...
func NewUIComponentFactory(queries *guicomponents.GUIQueries, pb *widgets.PanelBuilders, layout *widgets.LayoutConfig) *UIComponentFactory {
    return &UIComponentFactory{queries, pb, layout}
}

// Combat components
func (f *UIComponentFactory) CreateTurnOrderPanel() *widget.Container {
    return widgets.CreateStandardPanel(f.panelBuilders, "turn_order")
}

func (f *UIComponentFactory) CreateSquadListPanel() *widget.Container {
    return widgets.CreateStandardPanel(f.panelBuilders, "squad_list")
}

// Squad components
func (f *UIComponentFactory) CreateFormationGrid(onCellClick func(row, col int)) (*widget.Container, [3][3]*widget.Button) {
    // Grid creation logic
}

// Inventory components
func (f *UIComponentFactory) CreateItemListPanel() *widget.Container {
    return widgets.CreateStandardPanel(f.panelBuilders, "inventory_list")
}
```

**Pattern 2: Widget Builders (for simple, stateless widgets)**
```go
// widgets/builders.go - Consolidate all simple widget creation
package widgets

// Keep existing CreateButtonWithConfig, CreateTextAreaWithConfig, etc.
// Keep list helpers (CreateSquadList, CreateUnitList, etc.)
// These are simple, stateless builders - no factory needed
```

**Migration Path:**
1. Create `UIComponentFactory` in gui/core/
2. Move CombatUIFactory logic into UIComponentFactory.CreateCombatXXX methods
3. Move SquadBuilderUIFactory logic into UIComponentFactory.CreateSquadXXX methods
4. Delete old factory files
5. Update modes to use single factory instance

**Benefits:**
- Single factory pattern to learn
- Reduces 3 files to 1
- Clearer separation: Factory for complex UI, Builders for simple widgets
- Easier to find "where does X get created?"
