...
**Problem:** Position is stored in three places and manually synchronized:
1. Squad/Unit PositionComponent
2. GlobalPositionSystem spatial grid
3. Unit individual position components (if squad moves)

**Current Pattern (move_squad_command.go:85-98):**
```go
// ❌ Triple synchronization required
// 1. Update squad position
squadPos := common.GetComponentType[*coords.LogicalPosition](squadEntity, common.PositionComponent)
squadPos.X = newX
squadPos.Y = newY

// 2. Update position system
common.GlobalPositionSystem.RemoveEntity(squadID, *oldPos)
common.GlobalPositionSystem.AddEntity(squadID, *newPos)

// 3. Update all unit positions
for _, unitID := range unitIDs {
    unitPos := common.GetComponentType[*coords.LogicalPosition](unitEntity, common.PositionComponent)
    unitPos.X = newX
    unitPos.Y = newY
...
**Problems:**
- Easy to forget one synchronization step
- Data consistency bugs hard to debug
- Same pattern repeated in 2 locations

**Files Affected:**
- `squads/squadcommands/move_squad_command.go` (lines 85-98, 124-138)
- `combat/combatfactionmanager.go` (lines 56-70)

**Solution:** Create EntityManager.MoveEntity() wrapper

#### Implementation Steps

**Step 1:** Add MoveEntity wrapper (in `common/EntityManager.go`)
...
func (em *EntityManager) MoveEntity(
    ...
    // 1. Update component
    posComponent := em.GetComponent(entity, PositionComponent)
    if posComponent == nil {
        return fmt.Errorf("entity %d has no position component", entityID)
    }
...
// MoveSquadAndMembers moves squad and all member units
func (em *EntityManager) MoveSquadAndMembers(
    ...
    for _, unitID := range unitIDs {
        unitEntity := em.GetEntityByID(unitID)
        if unitEntity == nil {
            continue
        }
...
**Step 2:** Replace synchronization patterns (move_squad_command.go:85-98)
...
// ✅ AFTER
cmd.entityManager.MoveSquadAndMembers(
    squadID,
    squadEntity,
    unitIDs,
    *oldPos,
    coords.LogicalPosition{X: newX, Y: newY},
)
```

**Impact:**
- ✅ Atomic position updates (no partial syncs)
- ✅ Single source of truth for movement logic
- ✅ Easier to add future movement rules (collision, blocking, etc.)
- ✅ Reduced code duplication

**Effort:** 2 hours
**Risk:** Low (movement is localized)
**Testing:** Unit test movement with verification of all three storage locations
...
**Problem:** Inconsistent use of component access methods creates confusion about which to use when.

**Current Patterns:**
1. `GetComponentType[T](entity, component)` - Used after query ✅
2. `GetComponentTypeByID[T](manager, entityID, component)` - O(n) lookup ⚠️
3. `GetComponentTypeByIDWithTag[T](manager, entityID, tag, component)` - O(n) within tag ⚠️
...
**Effort:** 1 hour (documentation + review existing code)
...
**Problem:** Cleanup logic scattered across codebase; hard to extend.

**Solution:** Implement pre-dispose notification system

```go
// In EntityManager
type DisposeHook func(entity *ecs.Entity) error
...
// RegisterDisposeHook registers callback before entity disposal
func (em *EntityManager) RegisterDisposeHook(hook DisposeHook) {
    em.disposeHooks = append(em.disposeHooks, hook)
}

// CleanDisposeEntities now calls hooks automatically
func (em *EntityManager) CleanDisposeEntities(entity *ecs.Entity) error {
    // Call all registered hooks
    for _, hook := range em.disposeHooks {
        if err := hook(entity); err != nil {
            return fmt.Errorf("dispose hook failed: %w", err)
        }
    }
    ...
    // Update cache
    em.cacheMutex.Lock()
    delete(em.entityCache, entity.GetID())
    em.cacheMutex.Unlock()

    return nil
}

// Usage in gamesetup.go:
em.RegisterDisposeHook(func(entity *ecs.Entity) error {
    // Auto-cleanup position system
    if pos, ok := em.GetComponent(entity, PositionComponent); ok {
        GlobalPositionSystem.RemoveEntity(entity.GetID(), *pos.(*coords.LogicalPosition))
    }
    return nil
})
```
...
- ✅ Extensible (add more hooks without changing disposal code)
- ✅ Prevents future bugs (cleanup happens automatically)

**Effort:** 2 hours
**Risk:** Low (hooks are post-check)
