...
6.5. [ECS Views and Query Caching](#ecs-views-and-query-caching)
...
**Goal:** Initialize components in the ECS world using self-registration.

**File: `statuseffects/init.go`**
...
// Package-level view for status effect queries
var statusEffectView *ecs.View

func init() {
    common.RegisterSubsystem(func(em *common.EntityManager) {
        // Create component types
        StatusEffectComponent = em.World.NewComponent()

        // Create tags for querying
        StatusEffectTag = ecs.BuildTag(StatusEffectComponent)

        // Create views during init (not at runtime)
        statusEffectView = em.World.CreateView(StatusEffectTag)
    })
}
```

Components are automatically initialized when `common.InitializeSubsystems(manager)` is called during game startup. No manual wiring in `NewEntityManager()` needed.

**✅ Best Practices Applied:**
- Self-registration via `init()` + `common.RegisterSubsystem()`
- View created during subsystem init (not at runtime)
- Package-level view variable for internal queries
- Tags built with `ecs.BuildTag()` (not `manager.NewTag()`)
...
## ECS Views and Query Caching

TinkerRogue uses a **two-tier query API** that separates canonical queries from cached queries. This architecture gives you correctness-by-default for game logic while enabling high-performance paths for GUI and rendering hot loops.
...
A View is a pre-filtered query result maintained automatically by the ECS library. When components are added to or removed from entities, the corresponding View updates itself — no manual invalidation needed.

```go
// Creating a view (one-time O(n) cost)
squadView := manager.World.CreateView(SquadTag)

// Using the view (O(k) where k = matching entities, typically 10-50)
...
**Key properties:**
- **Automatic maintenance** — entities added/removed from View when components change
- **Thread-safe** — built-in `sync.RWMutex`
- **Zero manual invalidation** — library handles all updates
- **O(k) iteration** — where k = matching entities (vs O(n) for `World.Query()`)

### Package-Level Views

For queries used within a single package (not exposed to callers), declare views as package-level variables initialized during subsystem registration:

```go
// tactical/squads/squadmanager.go
var squadMemberView *ecs.View  // Package-level, unexported

func init() {
    common.RegisterSubsystem(func(em *common.EntityManager) {
        InitSquadComponents(em)
        InitSquadTags(em)
        squadMemberView = em.World.CreateView(SquadMemberTag)  // Created during init
    })
}
```

Package-level views are then used directly in query functions:

```go
// tactical/squads/squadqueries.go
func GetUnitIDsInSquad(squadID ecs.EntityID, squadmanager *common.EntityManager) []ecs.EntityID {
    var unitIDs []ecs.EntityID
    for _, result := range squadMemberView.Get() {  // Uses package-level view
        unitEntity := result.Entity
        memberData := common.GetComponentType[*SquadMemberData](unitEntity, SquadMemberComponent)
        if memberData.SquadID == squadID {
...
**Real examples of package-level views:**
- `tactical/squads/squadmanager.go` — `var squadMemberView *ecs.View` (unexported, used within squads package)
- `tactical/combat/combatcomponents.go` — `var factionView *ecs.View`, `var combatSquadView *ecs.View` (unexported, used within combat package)
- `overworld/core/init.go` — `var OverworldNodeView *ecs.View` (exported for cross-package use)

**When to export a package-level view:**
- Export (`OverworldNodeView`) when other packages need to iterate the same entity set
- Keep unexported (`factionView`) when the view is only used internally
...
|-------|--------------|------|---------|
| GetSquadEntity | 340us | 2.5us | **136x** |
| GetUnitIDsInSquad | 280us | 4us | **70x** |
| GetLeaderID | 200us | 1us | **200x** |

*Conditions: 20 squads, 9 units/squad, 1000+ total entities*

### When to Use Views

**Use Views when:**
- Query is called frequently (every frame, multiple times per frame)
- World has 1000+ entities (more entities = greater speedup)
- Query filters by specific tag (SquadTag, FactionTag, etc.)

**Don't use Views when:**
- Query is called once or twice total
- World is small (< 100 entities)
- Query has complex multi-component filtering that doesn't map to a single tag

### Best Practices

1. **Create views during `init()` registration** — not at runtime
...
### ECS Views (Performance Impact)

For a full treatment of Views, the two-tier query API, package-level views, and QueryCache structs, see the dedicated [ECS Views and Query Caching](#ecs-views-and-query-caching) section.
...
#### GetComponentTypeByID O(1) Optimization

Views alone aren't enough — you also need O(1) component lookups by EntityID:

```go
// common/ecsutil.go — Uses ECS library's O(1) entitiesByID map lookup
...
This change benefits the entire codebase — any code using `GetComponentTypeByID` gets O(1) instead of O(n).
...
├── squadroster.go         # Player-owned entity tracking
├── squads_test.go         # Tests
├── squadcache_test.go     # View benchmarks
├── squadservices/         # Service layer (business logic + validation)
│   ├── squad_builder_service.go
│   └── unit_purchase_service.go
└── squadcommands/         # Command pattern (undo/redo operations)
    ├── command.go             # SquadCommand interface
    ├── command_executor.go    # History + redo stack
...
type SquadCommand interface {
    Validate() error     // Pre-execution validation
    Execute() error      // Manager stored in struct
    Undo() error
    Description() string
}
...
func (cmd *RenameSquadCommand) Execute() error {
    squadData := common.GetComponentTypeByID[*squads.SquadData](
        cmd.manager, cmd.SquadID, squads.SquadComponent)
...
Services encapsulate business logic, wrapping ECS system functions with validation, error handling, and structured results. The service layer sits between the GUI and the ECS core.

#### Structured Result Types

Services return result structs instead of raw errors, providing rich feedback to callers:

```go
// tactical/squadservices/unit_purchase_service.go
...
```go
// tactical/combatservices/combat_service.go
...
**Pattern:** `Success bool` + `Error string` as common fields, plus domain-specific data. Callers check `result.Success` rather than `err != nil`.

#### Service Owning Subsystems

Complex services own and coordinate multiple ECS subsystems:
...
func NewCombatService(manager *common.EntityManager) *CombatService {
    ...
    return &CombatService{
        ...
        CombatCache:    cache,
        // ... other subsystems share the same cache
    }
}
```

**Pattern:** The service creates a shared `CombatQueryCache` and passes it to all subsystems, ensuring consistent View-based performance throughout.

#### Atomic Operations with Rollback

Services handle multi-step transactions with explicit rollback on failure:

```go
// tactical/squadservices/unit_purchase_service.go
func (ups *UnitPurchaseService) PurchaseUnit(playerID ecs.EntityID, template squads.UnitTemplate) *PurchaseResult {
    result := &PurchaseResult{UnitName: template.Name}
    ...
    // Step 2: Add to roster (rollback Step 1 on failure)
    if err := roster.AddUnit(unitEntity.GetID(), template.Name); err != nil {
        ups.entityManager.CleanDisposeEntity(unitEntity, nil)  // Rollback
        result.Error = fmt.Sprintf("failed to add to roster: %v", err)
        return result
    }

    // Step 3: Spend gold (rollback Steps 1+2 on failure)
    if err := resources.SpendGold(cost); err != nil {
        roster.RemoveUnit(unitEntity.GetID())                   // Rollback Step 2
        ups.entityManager.CleanDisposeEntity(unitEntity, nil)   // Rollback Step 1
    ...
    // Success
    result.Success = true
    result.UnitID = unitEntity.GetID()
...
**Pattern:** Each step has a corresponding rollback. If Step N fails, undo Steps N-1 through 1 in reverse order.

#### Simple Service (Basic Wrapper)

```go
// tactical/squadservices/squad_builder_service.go
type SquadBuilderService struct {
    entityManager *common.EntityManager
}

func (sbs *SquadBuilderService) PlaceUnit(
    squadID ecs.EntityID, unit UnitTemplate, gridRow, gridCol int,
) *PlaceUnitResult {
    result := &PlaceUnitResult{UnitName: unit.Name}
...
- Structured result types for rich error handling
- Atomic operations with explicit rollback
- GUI-friendly return types (no raw errors)
- Services own and coordinate subsystems

### ECS and Command Pattern Integration

Commands provide undo/redo support for squad management operations.

**Command Interface** (`tactical/squadcommands/command.go`):
```go
type SquadCommand interface {
    Validate() error     // Pre-execution validation
    Execute() error      // Perform operation (manager stored in struct, not passed)
    Undo() error         // Reverse operation using captured state
    Description() string // Human-readable description for logging
}
```

**Key design:** Commands store the `*EntityManager` reference in the struct itself — `Execute()` takes no parameters. This keeps the interface clean and allows commands to capture all context at creation time.

**Command Example:**
```go
// tactical/squadcommands/rename_squad_command.go
type RenameSquadCommand struct {
    manager *common.EntityManager
    SquadID ecs.EntityID
    NewName string
    OldName string  // Captured during Execute for Undo
}

func (cmd *RenameSquadCommand) Validate() error {
    entity := cmd.manager.FindEntityByID(cmd.SquadID)
    if entity == nil {
...
**CommandExecutor** (`tactical/squadcommands/command_executor.go`):

The executor manages history and redo stacks, returning structured `*CommandResult` values:
...
type CommandExecutor struct {
    history    []SquadCommand // Executed commands (most recent at end)
    redoStack  []SquadCommand // Undone commands available for redo
    maxHistory int
}

func (ce *CommandExecutor) Execute(cmd SquadCommand) *CommandResult {
    result := &CommandResult{Description: cmd.Description()}
    ...
    ce.redoStack = ce.redoStack[:0]  // New action invalidates redo
    result.Success = true
    return result
}
```

The executor uses slice-based history/redo stacks (not index-based navigation). `Undo()` pops from history and pushes to redo; `Redo()` does the reverse.

### Event-Driven GUI Caching

For turn-based games, frame-level cache rebuilds are wasteful — game state only changes on discrete events (damage, movement, turn changes). TinkerRogue uses an event-driven dirty-tracking pattern for GUI caches.

**SquadInfoCache** (`gui/framework/squadinfo_cache.go`):
...
func (sc *SquadInfoCache) GetSquadInfo(squadID ecs.EntityID) *SquadInfo {
    if sc.dirtySquads[squadID] || sc.cache[squadID] == nil {
        sc.cache[squadID] = sc.buildSquadInfo(squadID)  // Rebuild only when dirty
...
**Invalidation API:**
```go
// Mark single squad dirty — call on: damage, movement, action use, unit death
sc.MarkSquadDirty(squadID)

// Mark all squads dirty — call on: turn start/end, combat begin/end
sc.MarkAllDirty()

// Remove squad entirely — call on: squad destroyed or removed from game
sc.InvalidateSquad(squadID)
```

**GUIQueries composition** (`gui/framework/guiqueries.go`):
...
- **Squad movement** → `MarkSquadDirty(squadID)`
- **Turn change** → `MarkAllSquadsDirty()`
- **Unit death** → `MarkSquadDirty(squadID)`
- **Squad destroyed** → `InvalidateSquad(squadID)`
...
// Use with common.InitializeSubsystems(manager) to register all subsystem components.
...
- [ ] Views created during `init()` registration (not at runtime)
...
- [ ] Uses canonical query API (`*queries.go`) for logic, cached API (`*cache.go`) for hot paths
...
- [ ] Paired `*queries.go` + `*cache.go` files follow naming convention
- [ ] Cache file has header comment explaining canonical vs cached API
...
- [ ] Package-level views for frequently-queried tags
- [ ] QueryCache structs for cross-package view sharing
...
- [ ] `*cache.go` - View-based cached query functions
- [ ] `*system.go` - Only state-modifying logic
- [ ] `*roster.go` - Player-owned entity tracking (if applicable)
- [ ] `*_test.go` - Tests with proper setup
- [ ] Package contains one cohesive system

### Service Layer Checklist

- [ ] Result types use `Success bool` + `Error string` + domain fields
- [ ] Multi-step operations have explicit rollback on failure
- [ ] Service owns and coordinates its subsystems
- [ ] Shared QueryCache passed to all owned subsystems
- [ ] Validation separated from execution (`CanX()` + `DoX()`)

### Command Pattern Checklist

- [ ] Implements `SquadCommand` interface: `Validate()`, `Execute()`, `Undo()`, `Description()`
- [ ] Manager reference stored in struct (not passed to Execute)
- [ ] Old state captured during Execute for Undo
- [ ] CommandExecutor returns `*CommandResult` (not raw error)
