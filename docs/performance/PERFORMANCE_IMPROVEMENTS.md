...
### 1. Eliminate Image Allocations in DrawTileOverlay

**Current Hotspot:**
```
DrawTileOverlay: 1.07s cumulative (0.75% of total)
- ebiten.NewImage: 187ms per frame
- Image.Fill: 810ms per frame
```

**Problem:**
`DrawTileOverlay` creates a new `ebiten.Image` every single call:
```go
// CURRENT: Creates new image every frame for EVERY tile
func (vr *ViewportRenderer) DrawTileOverlay(screen *ebiten.Image, pos coords.LogicalPosition, fillColor color.Color) {
    rect := ebiten.NewImage(tileSize, tileSize)  // ALLOCATION!
    rect.Fill(fillColor)                          // GPU UPLOAD!
    // ...
}
```

This function is called for:
- Every valid movement tile (potentially 20-50 tiles per frame)
- During combat mode rendering
- For highlighting and selection overlays

**Solution:** Create an image pool/cache

```go
// Add to ViewportRenderer struct
type ViewportRenderer struct {
    centerPos       coords.LogicalPosition
    borderImages    BorderImageCache
    overlayCache    *ebiten.Image  // NEW: Reusable overlay image
    overlayTileSize int            // Track size for invalidation
}

// Modified DrawTileOverlay
func (vr *ViewportRenderer) DrawTileOverlay(screen *ebiten.Image, pos coords.LogicalPosition, fillColor color.Color) {
...
- Reduces image allocations from 20-50 per frame to 1 (one-time)
- Estimated improvement: 0.8-1.0s per frame (12-16 FPS gain at 60 FPS target)
- Memory pressure significantly reduced

**File to Modify:** `C:\Users\Afromullet\Desktop\TinkerRogue\gui\guimodes\guirenderers.go` (line 67-77)

---

### 2. Optimize DrawTileBorder - Stop Refilling Cached Images

**Current Hotspot:**
```
DrawTileBorder: 2.25s cumulative (1.57% of total)
- topBorder.Fill: 537ms
- bottomBorder.Fill: 451ms
- leftBorder.Fill: 454ms
- rightBorder.Fill: 89ms
Total Fill operations: ~1.5s
```

**Problem:**
Border images are cached but refilled with color on EVERY draw call:

```go
// CURRENT: Caches images but refills every call!
...
The caching is meaningless if we refill on every call. Additionally, creating 4 new `DrawImageOptions` instances is wasteful.

**Solution:** Use ColorScale instead of Fill
...
type ViewportRenderer struct {
    centerPos       coords.LogicalPosition
    borderImages    BorderImageCache
    borderDrawOpts  [4]ebiten.DrawImageOptions  // NEW: Reuse draw options
}

// Modified DrawTileBorder
func (vr *ViewportRenderer) DrawTileBorder(screen *ebiten.Image, pos coords.LogicalPosition, borderColor color.Color, thickness int) {
    screenX, screenY := vr.LogicalToScreen(pos)
    tileSize := vr.TileSize()

    // Get cached border images (creates white borders only once)
    topBorder, bottomBorder, leftBorder, rightBorder := vr.borderImages.GetOrCreate(tileSize, thickness)

    // Convert color to RGBA for ColorScale
    ...
    // Reuse draw options instead of creating 4 new ones
    // Top border
...
**Alternative (Better for many colors):** Pre-create colored borders

```go
type BorderImageCache struct {
    borders      map[color.Color][4]*ebiten.Image  // [top, bottom, left, right]
    tileSize     int
    thickness    int
}

func (cache *BorderImageCache) GetOrCreateColored(tileSize, thickness int, borderColor color.Color) ([4]*ebiten.Image, bool) {
    // Check if we need to recreate due to size change
    if cache.borders == nil || cache.tileSize != tileSize || cache.thickness != thickness {
        cache.borders = make(map[color.Color][4]*ebiten.Image)
        cache.tileSize = tileSize
        cache.thickness = thickness
    }

    // Return cached colored borders if they exist
    if borders, exists := cache.borders[borderColor]; exists {
        return borders, true
    }

    // Create new colored borders
    top := ebiten.NewImage(tileSize, thickness)
    bottom := ebiten.NewImage(tileSize, thickness)
    left := ebiten.NewImage(thickness, tileSize)
    right := ebiten.NewImage(thickness, tileSize)
    ...
    borders := [4]*ebiten.Image{top, bottom, left, right}
    cache.borders[borderColor] = borders
    return borders, false
}
```

**Expected Impact:** HIGH
- Eliminates 1.5s of Fill operations per frame
- Reduces DrawImageOptions allocations from 4 per call to 0
- Estimated improvement: 1.5-2.0s per frame (25-33 FPS gain)

**File to Modify:** `C:\Users\Afromullet\Desktop\TinkerRogue\gui\guimodes\guirenderers.go` (lines 14-112)
