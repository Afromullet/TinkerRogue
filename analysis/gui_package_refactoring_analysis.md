...
## 3. Code Quality Issues & Refactoring Opportunities

### 3.1 HIGH PRIORITY: combatmode.go (778 LOC)

#### Issue: Too Many Responsibilities
1. **Turn Order Management** - Tracking whose turn it is
2. **Faction Information Display** - Showing faction data
3. **Squad List Rendering** - Displaying available squads
4. **Combat State Machine** - Attack mode, target selection
5. **UI Control Flow** - Button clicks, mode transitions
6. **Combat Logging** - Log entries, message updates
7. **Rendering** - Drawing combat scene
...
type CombatMode struct {
    ...
    // UI widgets
    turnOrderLabel *ebitenui.Label
    squadListContainer *ebitenui.Container
    logTextArea *ebitenui.TextArea
    factionInfoPanel *ebitenui.Container
...
func (cm *CombatMode) HandleInput(input InputState) {
    // 200+ LOC of event handling
    if input.LeftClick {
        // 50+ LOC of click handling
    }
}

func (cm *CombatMode) Update() {
    // 150+ LOC of state updates
}
```

#### Proposed Extraction

##### 1. CombatLogManager (120 LOC)
**Responsibility:** Manage combat log entries and text updates
...
func (clm *CombatLogManager) GetDisplayText() string
func (clm *CombatLogManager) Update(turnManager *combat.TurnManager) string
```
...
##### 2. CombatStateManager (150 LOC)
**Responsibility:** Track combat state and transitions

```go
type CombatStateManager struct {
    selectedSquadID ecs.EntityID
    selectedTargetID ecs.EntityID
    attackMode AttackMode // enum: None, AttackMode, SelectTarget
}

func (csm *CombatStateManager) SetAttackMode(squadID ecs.EntityID) bool
func (csm *CombatStateManager) SetTarget(targetID ecs.EntityID) bool
...
- Easy to reason about valid states
- Testable state logic

##### 3. CombatUIFactory (200 LOC)
**Responsibility:** Build combat UI panels and widgets
...
func (cuf *CombatUIFactory) CreateTurnOrderPanel() *ebitenui.Container
func (cuf *CombatUIFactory) CreateSquadListPanel(filter SquadFilter) *ebitenui.Container
func (cuf *CombatUIFactory) CreateLogPanel() *ebitenui.Container
func (cuf *CombatUIFactory) CreateFactionInfoPanel(factionID ecs.EntityID) *ebitenui.Container
func (cuf *CombatUIFactory) CreateControlButtonRow() *ebitenui.Container
```

**Benefits:**
- Separates UI construction from mode logic
- Reusable panel creation
- Easier to maintain layouts
- Clear widget hierarchy

##### 4. Refactored CombatMode (200 LOC)
**Responsibility:** Orchestrate combat UI and state
...
type CombatMode struct {
    ...
    logContainer *ebitenui.Container
    squadListContainer *ebitenui.Container
}

func (cm *CombatMode) Initialize(ctx *UIContext) {
    cm.logManager = NewCombatLogManager()
    cm.stateManager = NewCombatStateManager()
    cm.uiFactory = NewCombatUIFactory(ctx.GUIQueries, ...)

    // Build initial UI
    cm.logContainer = cm.uiFactory.CreateLogPanel()
}
...
- Easy to follow flow
- Much shorter, easier to maintain

#### Impact Analysis
| Metric | Before | After |
|--------|--------|-------|
| Main file LOC | 778 | ~200 |
| Number of files | 1 | 4 |
| Avg file LOC | 778 | ~150 |
