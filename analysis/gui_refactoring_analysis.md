...
### 2. Panel + TextArea Initialization (MEDIUM Priority)
**Files:** ExplorationMode, InventoryMode, CombatMode, SquadDeploymentMode
**LOC:** 114
**Effort:** 2 hours
**Repetition Count:** 4+ identical patterns

**Current Pattern:**
```go
panel := panelBuilders.BuildPanel(...)
panelWidth := int(float64(layout.ScreenWidth) * 0.15)
panelHeight := int(float64(layout.ScreenHeight) * 0.2)
...
**Solution:** Extract `CreateDetailPanel(title, widthPct, heightPct) (*widget.Container, *TextArea)` helper function in modehelpers.go

---

### 3. Filter Button Containers (LOW Priority)
**Files:** InventoryMode, SquadDeploymentMode, FormationEditorMode
**LOC:** 60
**Effort:** 1.5 hours

**Current Issue:**
Identical pattern: `BuildPanel(TopLeft/TopRight, HorizontalRowLayout, Size, Padding)` repeated in multiple modes for filter buttons.

**Solution:** Extract `CreateFilterButtonContainer(alignment, initialButtons []*Button)` helper
